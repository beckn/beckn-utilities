...
export async function callBecknAPI(endpoint, payload) {
    try {
...
export function createBecknSearchRequest(params) {
...
export function processBecknSearchResponse(becknResponse) {
    ...
    try {
        ...
        if (becknResponse.message && becknResponse.message.catalog) {
            ...
            if (catalog.providers && Array.isArray(catalog.providers)) {
                result.providers = catalog.providers.map((provider) => {
                    ...
                    if (provider.items && Array.isArray(provider.items)) {
                        providerResult.items = provider.items.map((item) => {
                            return {
                                id: item.id || "unknown",
