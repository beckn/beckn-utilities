...
import { BecknSearchParamsSchemaShape, createBecknSearchRequest, processBecknSearchResponse, callBecknAPI } from "./beckn-utils.js";
...
server.tool("beckn-search", "Search for products or services across Beckn networks (e.g., 'Find pizza in Bangalore', 'Search for smartphones')", BecknSearchParamsSchemaShape, async (params) => {
    try {
        ...
        if (formattedResponse.providers && formattedResponse.providers.length > 0) {
            ...
            formattedResponse.providers.forEach((provider, index) => {
